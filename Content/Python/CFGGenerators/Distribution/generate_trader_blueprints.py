...
from Common.bpr_common import VANILLA_RANKS, get_weapon_families, load_json, load_weapon_progression, tier_blueprint_sid
...
def pool_group_key(trader_name: str) -> str:
    return sid_fragment(trader_name)


def pool_sid(trader_name: str) -> str:
    return f"BPR_TraderPool_{pool_group_key(trader_name)}"
...
def validate_config(weapon_config: dict, trader_config: dict) -> None:
    ...
    for trader_name, trader in traders.items():
        ...
        families = trader.get("families")
        if not isinstance(families, list) or not families:
            raise ValueError(f"Trader '{trader_name}' must define a non-empty 'families' list.")
        if len(families) != len(set(families)):
            raise ValueError(f"Trader '{trader_name}' contains duplicate weapon families.")
        unknown = sorted(set(families) - known_families)
        if unknown:
            raise ValueError(f"Trader '{trader_name}' contains unknown families: {', '.join(unknown)}")

        sid_fragment(trader_name)


def build_trader_families(trader: dict) -> list[str]:
    return list(trader["families"])
...
def main() -> None:
    ...
    for trader_name, trader in trader_config["traders"].items():
        families = build_trader_families(trader)
        ...
        if not blueprints:
            raise ValueError(f"Trader '{trader_name}' has an empty blueprint pool.")

        group = pool_group_key(trader_name)
