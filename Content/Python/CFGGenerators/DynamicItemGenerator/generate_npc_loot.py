...
def validate(weapon: dict, npc: dict) -> None:
    ...
    for target in npc["targets"]:
        ...
        if not match:
            raise ValueError(
                f"Unsupported NPC target name: {target}"
            )
        ...
        if faction not in npc["faction_max_rank"]:
            raise ValueError(
                f"No faction_max_rank for {faction}"
            )

        if role not in npc["role_allowed_families"]:
            raise ValueError(
                f"No role allowlist for {role}"
            )
...
def cumulative_families(
    ...
    for current in RANK_ORDER:
        result.extend(
            weapon["weapon_progression"][current]
        )
...
def effective_families(
    ...
    return [
        family
        for family in cumulative_families(
            weapon,
            effective_rank,
        )
        if family in unlocked
        and family in role_allowed
    ]
...
def pool_prefix(
    ...
    return (
        f"BPR_NPC_"
        f"{safe_sid_part(faction)}_"
        f"{safe_sid_part(role)}_"
        f"{rank}"
    )
...
def generate_pools(
    ...
    for target in npc["targets"]:
        ...
        for rank in RANK_ORDER:
            ...
            # If a combination contains no possible families,
            # do not emit pools.
...
def main() -> None:
    ...
    print("----------------------")
    print(
        f"Weapon config: {WEAPON_CONFIG_FILE}"
    )
    print(
        f"NPC config:    {NPC_CONFIG_FILE}"
    )
    print()
    print(
        f"Generated: {POOLS_OUTPUT}"
    )
    print(
        f"Generated: {PATCH_OUTPUT}"
    )
    print()
    print(
        f"NPC targets: {len(npc['targets'])}"
    )
    print("Drop chances:")
    print(
        f"  Tier I:      "
        f"{npc['drop_chances']['tier_1']:.2%}"
    )
    print(
        f"  Tier II:     "
        f"{npc['drop_chances']['tier_2']:.2%}"
    )
    print(
        f"  Tier III:    "
        f"{npc['drop_chances']['tier_3']:.2%}"
    )
    print(
        f"  Upgrade Kit: "
        f"{npc['drop_chances']['upgrade_kit']:.2%}"
    )
